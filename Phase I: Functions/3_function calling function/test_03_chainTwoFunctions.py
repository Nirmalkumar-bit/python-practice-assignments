...
def _load_module(path: Path):
    if not path.exists():
        raise AssertionError(f"expected output: GRACE\nactual output: <missing file: {path.name}>")
    spec = importlib.util.spec_from_file_location("student_mod_03", str(path))
...
def test_stdout_exact(capsys):
    path = Path(__file__).resolve().parent / "03_chainTwoFunctions.py"
    _load_module(path)
