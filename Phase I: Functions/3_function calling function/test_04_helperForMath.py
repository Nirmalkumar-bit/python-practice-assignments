...
def _load_module(path: Path):
    if not path.exists():
        raise AssertionError(f"expected output: 25\nactual output: <missing file: {path.name}>")
    spec = importlib.util.spec_from_file_location("student_mod_04", str(path))
...
def test_stdout_exact(capsys):
    path = Path(__file__).resolve().parent / "04_helperForMath.py"
    _load_module(path)
