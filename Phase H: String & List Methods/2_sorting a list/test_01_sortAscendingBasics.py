...
def test_sort_ascending_in_place_and_stdout_exact():
    target = Path(__file__).resolve().parent / "01_sortAscendingBasics.py"
    buf = io.StringIO()
    with contextlib.redirect_stdout(buf):
        mod = load_module_from_path("a01", target)
    ...
    if getattr(mod, "nums", None) != [1, 2, 3, 4, 5]:
        raise AssertionError(f"expected output: {expected_out}actual output: {actual_out}")
