...
def test_sort_descending_and_stdout_exact():
    target = Path(__file__).resolve().parent / "02_sortDescendingBasics.py"
    buf = io.StringIO()
    with contextlib.redirect_stdout(buf):
        mod = load_module_from_path("a02", target)
    expected_out = "[9, 7, 5, 3, 1]\n"
    actual_out = buf.getvalue()
    if actual_out != expected_out:
        raise AssertionError(f"expected output: {expected_out}actual output: {actual_out}")
    if getattr(mod, "nums", None) != [9, 7, 5, 3, 1]:
        raise AssertionError(f"expected output: {expected_out}actual output: {actual_out}")
