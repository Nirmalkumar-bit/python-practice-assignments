...
def test_sort_only_subset_in_place_and_stdout_exact():
    target = Path(__file__).resolve().parent / "09_sortOnlySubsetInPlace.py"
    buf = io.StringIO()
    with contextlib.redirect_stdout(buf):
        mod = load_module_from_path("a09", target)
    expected_list = [100, 50, 1, 2, 3, 4, 9, 8, 7]
    ...
    if getattr(mod, "nums", None) != expected_list:
        raise AssertionError(f"expected output: {expected_out}actual output: {actual_out}")
